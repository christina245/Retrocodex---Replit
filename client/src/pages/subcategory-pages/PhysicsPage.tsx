...
export default function PhysicsPage() {
  ...
  const allFacts: CategoryFact[] = useMemo(() => {
    return dbFacts
      .filter(fact => fact.categories.includes("Physics"))
      .map(fact => ({
        id: fact.id,
    ...
    <div className="physics-page">
      ...
      <main className="physics-main-content">
        ...
        <div className="physics-header-section">
          <p className="physics-intro-text">Misconceptions about</p>
          <h1 className="physics-title">Physics</h1>
        </div>
          ...
          <div className="physics-content-container">
            <div className="physics-facts-grid">
              {displayedFacts.length > 0 ? (
                ...
                <p className="physics-no-facts">No facts available yet. Check back soon!</p>
              )}
            </div>

            <aside className="physics-sidebar">
              <EmailSignupBanner 
                onSubmit={(email) => handleEmailSubmit(email, "physics-page")} 
              />
            </aside>
          </div>
        </div>
